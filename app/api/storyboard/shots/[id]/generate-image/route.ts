...
export async function POST(request: Request, { params }: Params) {
  try {
    ...
    // Fetch parent scene info if available
    ...
    // Fetch project characters with approved or pending sheets
    const projectCharacters = await db
      .select()
      .from(storyboardCharacters)
      .where(eq(storyboardCharacters.projectId, project.id));

    // Filter characters relevant to this shot (tagged or mentioned in description/dialogue)
    ...
    try {
      ...
      return NextResponse.json(
        {
          error: genError.message || "Shot image generation failed",
          code: isSafety ? "SAFETY_FILTER_FLAGGED" : "GENERATION_FAILED",
    ...
    console.error("[POST Generate Shot Image] Error:", error);
    return NextResponse.json(
      { error: error.message || "Failed to generate shot image" },
      { status: 500 },
    );
  }
}
